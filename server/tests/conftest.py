...
def allow_auth(monkeypatch):
    monkeypatch.setattr('server.api.validate_auth_token', lambda task_id, token: True)


@pytest.fixture
def deny_auth(monkeypatch):
    monkeypatch.setattr('server.api.validate_auth_token', lambda task_id, token: False)
...
def mock_read_message(monkeypatch):
    monkeypatch.setattr('server.api.read_message', lambda task_id: _MOCK_MESSAGE)
    return _MOCK_MESSAGE


@pytest.fixture
def mock_read_message_empty(monkeypatch):
    monkeypatch.setattr('server.api.read_message', lambda task_id: None)


@pytest.fixture
def mock_update_task(monkeypatch):
    valid = {'STARTED', 'RUNNING', 'PAUSED', 'COMPLETE', 'ERROR', 'TIME_OUT'}
    monkeypatch.setattr('server.api.update_task', lambda task_id, state: state in valid)


@pytest.fixture
def mock_update_message(monkeypatch):
    monkeypatch.setattr(
        'server.api.update_message',
...
def mock_unregister(monkeypatch):
    monkeypatch.setattr('server.api.unregister', lambda client_id: True)


@pytest.fixture
def mock_list_clients(monkeypatch):
    monkeypatch.setattr('server.api.list_clients', lambda auth: [_MOCK_CLIENT])
    return [_MOCK_CLIENT]
...
def mock_list_actions(monkeypatch):
    ...
    monkeypatch.setattr('server.api.list_actions', lambda task_id, auth: actions)
    return actions


@pytest.fixture
def mock_read_status(monkeypatch):
    monkeypatch.setattr('server.api.read_status', lambda task_id, count: [_MOCK_STATUS])
    return [_MOCK_STATUS]


@pytest.fixture
def mock_post_message(monkeypatch):
    monkeypatch.setattr('server.api.post_message', lambda task_id, payload: MESSAGE_ID)


@pytest.fixture
def mock_list_messages(monkeypatch):
    monkeypatch.setattr('server.api.list_messages', lambda task_id: [_MOCK_MESSAGE])
    return [_MOCK_MESSAGE]


@pytest.fixture
def mock_delete_task(monkeypatch):
    monkeypatch.setattr('server.api.delete_task', lambda task_id: True)


@pytest.fixture
def mock_delete_all(monkeypatch):
    monkeypatch.setattr('server.api.delete_all', lambda payload=None: True)
